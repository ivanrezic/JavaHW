...
public class Rectangle {
	...
	public static void main(String[] args) {
		float sirinaPravokutnika, visinaPravokutnika;

		if (args.length == 1 || args.length > 2) {
			System.out.println("Preko naredbenog retka moraju biti unesena točno 2 argumenta!");
		} else if (args.length == 2) {
			try {
				sirinaPravokutnika = Float.parseFloat(args[0]);
				visinaPravokutnika = Float.parseFloat(args[1]);
				if (visinaPravokutnika < 0 || sirinaPravokutnika < 0) {
					System.out.println("Brojevi moraju biti veci od ili jednaki 0");
				}
				System.out.println("Pravokutnik širine " + sirinaPravokutnika + " i visine " + visinaPravokutnika
						+ " ima površinu " + sirinaPravokutnika * visinaPravokutnika + " te opseg "
						+ (2 * sirinaPravokutnika + 2 * visinaPravokutnika));
			...
			sirinaPravokutnika = postaviStranicu("sirinu",sc);
			visinaPravokutnika = postaviStranicu("visinu",sc);
	...
	/**
	 * Metoda koja vraca stranicu u decimalnom obliku te provjerava jel zadovoljava zadane kriterije
	 * 
	 * @param stranica Argument koji oblije pitanje prilikom pokretanja programa npr. ako je stranica = "visinu"
	 * program ce ispisivati "Unesite visinu"
	 * @param sc referenca na objekt Scanner
