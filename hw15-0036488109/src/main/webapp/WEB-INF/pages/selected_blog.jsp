<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>

<!DOCTYPE html>
<html>
	<style type="text/css">	
		.logout{
			float: right;
			margin-right: 5px;
		}
	</style>
<head>
</head>
<body bgcolor="#d3d2b5">

	<c:choose>
		<c:when test="${currentUser == null}">
			Not logged in.<hr>
		</c:when>
		<c:otherwise>
			<b>Name:</b> ${currentUser.firstName} <b>Surname:</b> ${currentUser.lastName} <b>Nickname:</b> ${currentUser.nick}
			<b class="logout"><a href="/blog/servleti/main?logout">Logout</a></b>
			<hr>
		</c:otherwise>
	</c:choose>
	
	<p>${blog.title}: ${blog.text}<p>
	<c:choose>
		<c:when test="${currentUser.nick.equals(selectedUser)}">
			<a href="${blog.id}/edit">Edit blog</a>
		</c:when>
	</c:choose>
	<hr>
	
	
	<c:forEach var="comment" items="${comments}">
		<b>${comment.postedOn} - ${comment.usersEMail}</b>
		<br>
		${comment.message}
		<br>
		<br>
	</c:forEach>	

	<form action="${blog.id}" method="POST">
		Email: <br><input type="email" name="email" placeholder="your email.."> <br>
		Comment: <br><textarea name="text" rows="3" cols="40"></textarea><br>
		<input type="submit" value="Post">
		<input type="reset" value="Reset">
	</form>

</body>
</html>